 ...
 * 知识闯关页面
 */
import { generateQuestionsFromHalls, selectRandomQuestions } from '../../utils/questionGenerator';

// 展馆数据（与exhibition-halls页面保持一致）
const HALLS_DATA = [
  {
    id: '1',
    name: '古代监狱',
...
Page({
  data: {
    questions: [] as any[],
    currentIndex: 0,
    answers: {} as Record<number, number>, // 用户选择的答案索引
    selectedAnswer: null as number | null, // 当前题目选中的答案
    showResult: false,
    scrollHeight: '550px',
  ...
  loadQuestions() {
    try {
      // 从展馆数据生成所有题目
      const allQuestions = generateQuestionsFromHalls(HALLS_DATA);
      // 随机选择10道题
   ...
   * 选择答案
   */
  onSelectAnswer(e: WechatMiniprogram.TouchEvent) {
  ...
  onPrevQuestion() {
    ...
    if (currentIndex > 0) {
      ...
      this.setData({
        ...
        selectedAnswer: this.data.answers[prevIndex] ?? null,
      });
      this.updateCurrentQuestion();
    }
  },
  ...
  onSubmit() {
    ...
    // 跳转到结果页面
    wx.redirectTo({
      url: `/pages/knowledge-result/knowledge-result?score=${score}&total=${totalQuestions}&correct=${correctCount}&passed=${passed ? 1 : 0}`,
    });
  },
});
